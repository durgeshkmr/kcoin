...
class ZerocoinSpendTest(KcoinTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zkcn_bal, kcn_bal):
            zkcn_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zkcn_bal)
            kcn_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], kcn_bal)
            return zkcn_bal, kcn_bal
        ...
        zkcn_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zkcn_balance, 6666)
        ...
        zkcn_balance, balance = check_balances(denom_2, zkcn_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zkcn_balance, balance = check_balances(denom_3, zkcn_balance, balance)
        ...
        assert_raises_rpc_error(-26, "bad-txns-invalid-zkcn",
        ...
        _, _ = check_balances(denom_1, zkcn_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
