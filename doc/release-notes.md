...
KCOIN Core version *version* is now available from:  <https://github.com/kcoin-project/kcoin/releases>
...
Enabling major improvements over the keystore management, the KCOIN wallet doesn't require regular backups as before, keys are following a deterministic creation path that can be verified at any time (before HD Wallet, every keypair was randomly created and added to the keypool, forcing the user to backup the wallet every certain amount of time or could end up loosing coins forever if the latest `wallet.dat` was not being used).
...
An extended description of this large work can be found in the PR [here](https://github.com/KCOIN-Project/KCOIN/pull/1327).
...
Automatic zKCN backup has been disabled. Thus, the following configuration options have been removed  (either as entries in the kcoin.conf file or as startup flags):
- `autozkcnbackup`
- `backupzkcn`
- `zkcnbackuppath`
...
The default value for the stake-split threshold has been lowered from 2000 KCN, down  to 500 KCN.


Dependencies
------------

The minimum required version of QT has been increased from 5.0 to 5.5.1 (the [depends system](https://github.com/kcoin-project/kcoin/blob/master/depends/README.md) provides 5.9.7)
...
The p2p alert system has been removed in [#1372](https://github.com/KCOIN-Project/KCOIN/pull/1372) and the 'alert' message is no longer supported.
...
The `-reservebalance` configuration/startup option has been removed ([PR #1373](https://github.com/KCOIN-Project/KCOIN/pull/1373)).
