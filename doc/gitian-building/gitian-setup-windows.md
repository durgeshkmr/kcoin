...
The build script we'll be using is contained in the KCOIN github repository ([contrib/gitian-build.py](https://github.com/kcoin-project/kcoin/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the KCOIN repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/KCOIN-Project/KCOIN/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, kcoin-detached-sigs, and kcoin GitHub repos.
