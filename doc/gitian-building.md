...
*Setup instructions for a gitian build of KCOIN Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the KCOIN
...
to the KCOIN GitHub release page.
...
- [Building KCOIN Core](#building-kcoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/kcoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/kcoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/kcoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/kcoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/kcoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/kcoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/kcoin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@kcoin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/kcoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/kcoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/kcoin-osx-*-build.assert
```
