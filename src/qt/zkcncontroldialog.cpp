...
#include "ui_zkcncontroldialog.h"
...
std::set<std::string> ZKcnControlDialog::setSelectedMints;
std::set<CMintMeta> ZKcnControlDialog::setMints;

bool CZKcnControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZKcnControlDialog::COLUMN_DENOMINATION || column == ZKcnControlDialog::COLUMN_VERSION || column == ZKcnControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZKcnControlDialog::ZKcnControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZKcnControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zKCN Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZKcn->setProperty("cssClass", "text-main-purple");
    ui->labelZKcn_int->setProperty("cssClass", "text-main-purple");
...
ZKcnControlDialog::~ZKcnControlDialog()
{
    delete ui;
}

void ZKcnControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZKcnControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZKcnControlWidgetItem* itemDenom(new CZKcnControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZKcnControlWidgetItem *itemMint = new CZKcnControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < nRequiredConfs)
                ...
                strReason = "Your wallet is locked. Impossible to spend zKCN.";
            else if (!mint.isSeedCorrect)
                strReason = "The zKCN seed used to mint this zKCN is not the same as currently hold in the wallet";
            else
...
void ZKcnControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZKcnControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZKcn_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZKcnControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZKcnControlDialog::GetSelectedMints()
{
...
void ZKcnControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
