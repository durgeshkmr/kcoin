...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::KCN);
    ...
    if (settings.contains("nAnonymizeKcoinAmount") || reset)
        SoftSetArg("-anonymizekcoinamount", settings.value("nAnonymizeKcoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in kcoin.cpp during shotdown to also remove the window positions
