...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    Q_FOREACH (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered kcoin address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > sporkManager.GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            Q_EMIT message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 KCN.").arg(sporkManager.GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        Q_FOREACH (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal kcoin:URI (kcoin:XyZ...?message=example)
            {
...
bool WalletModel::createZkcnSpend(
        CWalletTx &wtxNew,
...
bool WalletModel::convertBackZkcn(
        CAmount value,
        std::vector<CZerocoinMint> &vMintsSelected,
        CZerocoinSpendReceipt &receipt
){
...
static void NotifyzKCNReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzKCNReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzKCNReset.connect(boost::bind(NotifyzKCNReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzKCNReset.disconnect(boost::bind(NotifyzKCNReset, this));
...
bool WalletModel::updateAddressBookPurpose(const QString &addressStr, const std::string& purpose) {
    ...
    if (address.IsStakingAddress())
        return error("Invalid KCOIN address, cold staking address");
    CKeyID keyID;
...
bool WalletModel::getKeyId(const CBitcoinAddress& address, CKeyID& keyID) {
    if (!address.IsValid())
        return error("Invalid KCOIN address");

    if (!address.GetKeyID(keyID))
        return error("Unable to get KeyID from KCOIN address");
...
{
    setMints.clear();
    setMints = pwalletMain->zkcnTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
