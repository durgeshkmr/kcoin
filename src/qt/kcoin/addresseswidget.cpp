...
#include "qt/kcoin/addresseswidget.h"
#include "qt/kcoin/forms/ui_addresseswidget.h"
#include "qt/kcoin/addresslabelrow.h"
#include "qt/kcoin/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(KCOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter KCOIN address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress kcnAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(kcnAdd.Get(), label.toUtf8().constData(),
                kcnAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
