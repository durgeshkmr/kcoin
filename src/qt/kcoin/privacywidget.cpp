...
#include "qt/kcoin/privacywidget.h"
#include "qt/kcoin/forms/ui_privacywidget.h"
#include "qt/kcoin/qtutils.h"
#include "guiutil.h"
#include "qt/kcoin/denomgenerationdialog.h"
#include "qt/kcoin/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(KCOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zKCN anonymizes your KCN by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zKCN or convert back to KCN"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 KCN ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zKCN");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zKCN");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zKCN");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zKCN");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zKCN");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zKCN");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zKCN");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zKCN");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zKCN Movements"));
    ...
    // Only Convert to KCN enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzKCN->setTitleClassAndText("btn-title-grey", "Total 0 zKCN");
    ui->btnTotalzKCN->setSubTitleClassAndText("text-subtitle", "Show denominations of zKCN owned.");
    ui->btnTotalzKCN->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select KCN outputs to mint into zKCN.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zKCN");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzKCN, SIGNAL(clicked()), this, SLOT(onTotalZkcnClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zKCN");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of KCN to mint into zKCN"));
    }else{
        btnText = tr("Convert back to KCN");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zKCN to convert back into KCN"));
    }
...
void PrivacyWidget::onTotalZkcnClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzKCN->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzKCN->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zKCN is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zKCN, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zKCN").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zKCN minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZkcn(
            value,
            selectedMints,
            receipt
    )){
        ...
        // Spend succeed
        inform(tr("zKCN converted back to KCN"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zKCN </b>";
    ...
    ui->btnTotalzKCN->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
