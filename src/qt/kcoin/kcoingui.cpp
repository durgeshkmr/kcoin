// Copyright (c) 2019-2020 The KCOIN developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "qt/kcoin/kcoingui.h"
...
#include "qt/kcoin/qtutils.h"
#include "qt/kcoin/defaultdialog.h"
#include "qt/kcoin/settings/settingsfaqwidget.h"
...
KCOINGUI::KCOINGUI(const NetworkStyle* networkStyle, QWidget* parent) :
        QMainWindow(parent),
        clientModel(0){
...
void KCOINGUI::createActions(const NetworkStyle* networkStyle){
...
void KCOINGUI::connectActions() {
    ...
    connect(topBar, &TopBar::showHide, this, &KCOINGUI::showHide);
    connect(topBar, &TopBar::themeChanged, this, &KCOINGUI::changeTheme);
    connect(topBar, &TopBar::onShowHideColdStakingChanged, navMenu, &NavMenuWidget::onShowHideColdStakingChanged);
    connect(settingsWidget, &SettingsWidget::showHide, this, &KCOINGUI::showHide);
    connect(sendWidget, &SendWidget::showHide, this, &KCOINGUI::showHide);
    connect(receiveWidget, &ReceiveWidget::showHide, this, &KCOINGUI::showHide);
    connect(addressesWidget, &AddressesWidget::showHide, this, &KCOINGUI::showHide);
    connect(masterNodesWidget, &MasterNodesWidget::showHide, this, &KCOINGUI::showHide);
    connect(masterNodesWidget, &MasterNodesWidget::execDialog, this, &KCOINGUI::execDialog);
    connect(coldStakingWidget, &ColdStakingWidget::showHide, this, &KCOINGUI::showHide);
    connect(coldStakingWidget, &ColdStakingWidget::execDialog, this, &KCOINGUI::execDialog);
    connect(settingsWidget, &SettingsWidget::execDialog, this, &KCOINGUI::execDialog);
}
...
void KCOINGUI::handleRestart(QStringList args){
...
void KCOINGUI::messageInfo(const QString& text){
...
void KCOINGUI::message(const QString& title, const QString& message, unsigned int style, bool* ret) {
    QString strTitle =  tr("KCOIN Core"); // default title
...
bool KCOINGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn){
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("KCOIN Core"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
...
void KCOINGUI::goToSend(){
    showTop(sendWidget);
}

void KCOINGUI::goToAddresses(){
    showTop(addressesWidget);
}

void KCOINGUI::goToPrivacy(){
    if (privacyWidget) showTop(privacyWidget);
}

void KCOINGUI::goToMasterNodes(){
    showTop(masterNodesWidget);
}

void KCOINGUI::goToColdStaking(){
    showTop(coldStakingWidget);
}

void KCOINGUI::goToSettings(){
    showTop(settingsWidget);
}

void KCOINGUI::goToReceive(){
    showTop(receiveWidget);
}

void KCOINGUI::showTop(QWidget* view){
...
void KCOINGUI::changeTheme(bool isLightTheme){
...
void KCOINGUI::resizeEvent(QResizeEvent* event){
    // Parent..
...
bool KCOINGUI::execDialog(QDialog *dialog, int xDiv, int yDiv){
    return openDialogWithOpaqueBackgroundY(dialog, this);
}

void KCOINGUI::showHide(bool show){
...
int KCOINGUI::getNavWidth(){
    return this->navMenu->width();
}

void KCOINGUI::openFAQ(int section){
    showHide(true);
...
bool KCOINGUI::addWallet(const QString& name, WalletModel* walletModel)
{
    ...
    if (walletModel->getZerocoinBalance() > 0) {
        ...
        connect(privacyWidget, &PrivacyWidget::message, this, &KCOINGUI::message);
        connect(privacyWidget, &PrivacyWidget::showHide, this, &KCOINGUI::showHide);
    }

    // Connect actions..
    connect(masterNodesWidget, &MasterNodesWidget::message, this, &KCOINGUI::message);
    connect(coldStakingWidget, &MasterNodesWidget::message, this, &KCOINGUI::message);
    connect(topBar, &TopBar::message, this, &KCOINGUI::message);
    connect(sendWidget, &SendWidget::message,this, &KCOINGUI::message);
    connect(receiveWidget, &ReceiveWidget::message,this, &KCOINGUI::message);
    connect(addressesWidget, &AddressesWidget::message,this, &KCOINGUI::message);
    connect(settingsWidget, &SettingsWidget::message, this, &KCOINGUI::message);
...
void KCOINGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address) {
