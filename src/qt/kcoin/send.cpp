...
#include "qt/kcoin/send.h"
#include "qt/kcoin/forms/ui_send.h"
#include "qt/kcoin/addnewcontactdialog.h"
#include "qt/kcoin/qtutils.h"
#include "qt/kcoin/sendchangeaddressdialog.h"
#include "qt/kcoin/optionbutton.h"
#include "qt/kcoin/sendconfirmdialog.h"
#include "qt/kcoin/myaddressrow.h"
#include "qt/kcoin/guitransactionsutils.h"
...
#include "zkcncontroldialog.h"

SendWidget::SendWidget(KCOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("KCN");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zKCN");
    setCssProperty(ui->pushRight, "btn-check-right");

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (KCN) or private coins (zKCN)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("KCOIN address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 KCN");
    ...
    setCssProperty(coinIcon, "coin-icon-kcn");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onKCNSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onKCNSelected(false);});
...
void SendWidget::refreshAmounts() {
    ...
    bool isZkcn = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZkcn));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZkcn ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendKcn = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendKcn ? AskPassphraseDialog::Context::Send_KCN : AskPassphraseDialog::Context::Send_zKCN, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendKcn) ? send(recipients) : sendZkcn(recipients)) {
...
bool SendWidget::sendZkcn(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        Q_EMIT message(tr("Spend Zerocoin"), tr("zKCN is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zKCN selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!ZKcnControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZKcnControlDialog::GetSelectedMints();
    ...
    if (walletModel->sendZkcn(
        ...
        inform(tr("zKCN transaction sent!"));
        ZKcnControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZKCN_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zKCN require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().GetConsensus().ZC_MaxSpendsPerTx; // Maximum possible spends for one zKCN transaction
        ...
        Q_EMIT message("zKCN transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isKCN){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any KCN to select."));
        }
    }else{
        if (walletModel->getZerocoinBalance() > 0) {
            ZKcnControlDialog *zKcnControl = new ZKcnControlDialog(this);
            zKcnControl->setModel(walletModel);
            zKcnControl->exec();
            ui->btnCoinControl->setActive(!ZKcnControlDialog::setSelectedMints.empty());
            zKcnControl->deleteLater();
        } else {
            inform(tr("You don't have any zKCN in your balance to select."));
        }
    }
}
...
void SendWidget::onKCNSelected(bool _isKCN){
    isKCN = _isKCN;
    setCssProperty(coinIcon, _isKCN ? "coin-icon-kcn" : "coin-icon-zkcn");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress kcnAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(kcnAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(kcnAdd.Get(), dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
