...
#include "qt/kcoin/dashboardwidget.h"
#include "qt/kcoin/forms/ui_dashboardwidget.h"
#include "qt/kcoin/sendconfirmdialog.h"
#include "qt/kcoin/txrow.h"
...
DashboardWidget::DashboardWidget(KCOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of KCN and zKCN staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareKcn, "square-chart-kcn");
    setCssProperty(ui->labelSquarezKcn, "square-chart-zkcn");
    setCssProperty(ui->labelKcn, "text-chart-kcn");
    setCssProperty(ui->labelZkcn, "text-chart-zkcn");
    ...
    ui->labelAmountZkcn->setText("0 zKCN");
    ui->labelAmountKcn->setText("0 KCN");
    setCssProperty(ui->labelAmountKcn, "text-stake-kcn-disable");
    setCssProperty(ui->labelAmountZkcn, "text-stake-zkcn-disable");
    ...
    ui->btnHowTo->setText(tr("How to get KCN or zKCN"));
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("KCN")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isKcn = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZKCN;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair KCN, zKCN
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            kcn = (pair.first != 0) ? pair.first / 100000000 : 0;
            zkcn = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalKcn += pair.first;
            chartData->totalZkcn += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesKcn.append(kcn);
        chartData->valueszKcn.append(zkcn);

        int max = std::max(kcn, zkcn);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("KCN");
    set1 = new QBarSet("zKCN");
    ...
    set0->append(chartData->valuesKcn);
    set1->append(chartData->valueszKcn);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalKcn > 0 || chartData->totalZkcn > 0) {
        setCssProperty(ui->labelAmountKcn, "text-stake-kcn");
        setCssProperty(ui->labelAmountZkcn, "text-stake-zkcn");
    } else {
        setCssProperty(ui->labelAmountKcn, "text-stake-kcn-disable");
        setCssProperty(ui->labelAmountZkcn, "text-stake-zkcn-disable");
    }
    forceUpdateStyle({ui->labelAmountKcn, ui->labelAmountZkcn});
    ui->labelAmountKcn->setText(GUIUtil::formatBalance(chartData->totalKcn, nDisplayUnit));
    ui->labelAmountZkcn->setText(GUIUtil::formatBalance(chartData->totalZkcn, nDisplayUnit, true));
