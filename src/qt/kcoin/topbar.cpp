...
#include "qt/kcoin/topbar.h"
#include "qt/kcoin/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/kcoin/lockunlock.h"
#include "qt/kcoin/qtutils.h"
#include "qt/kcoin/receivedialog.h"
...
TopBar::TopBar(KCOINGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    std::initializer_list<QWidget*> lblTitles = {ui->labelTitle1, ui->labelTitleAvailablezKcn, ui->labelTitle3, ui->labelTitle4, ui->labelTitlePendingzKcn, ui->labelTitleImmaturezKcn};
    ...
    setCssProperty({ui->labelAmountTopKcn, ui->labelAmountTopzKcn}, "amount-small-topbar");
    setCssProperty({ui->labelAmountKcn, ui->labelAvailablezKcn}, "amount-topbar");
    setCssProperty({ui->labelPendingKcn, ui->labelPendingzKcn, ui->labelImmatureKcn, ui->labelImmaturezKcn}, "amount-small-topbar");
...
void TopBar::loadWalletModel() {
    ...
    // update the display unit, to not use the default ("KCOIN")
    updateDisplayUnit();
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    // KCN Total
    CAmount kcnAvailableBalance = balance;
    // zKCN Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalKcn = GUIUtil::formatBalance(kcnAvailableBalance, nDisplayUnit);
    QString totalzKcn = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);

    // KCN
    // Top
    ui->labelAmountTopKcn->setText(totalKcn);
    // Expanded
    ui->labelAmountKcn->setText(totalKcn);
    ui->labelPendingKcn->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelImmatureKcn->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));

    // Update display state and/or values for zKCN balances as necessary
    bool fHaveZerocoins = zerocoinBalance > 0;

    // Set visibility of zKCN label titles/values
    ui->typeSpacerTop->setVisible(fHaveZerocoins);
    ui->typeSpacerExpanded->setVisible(fHaveZerocoins);
    ui->labelAmountTopzKcn->setVisible(fHaveZerocoins);
    ui->zerocoinBalances->setVisible(fHaveZerocoins);

    // Top
    ui->labelAmountTopzKcn->setText(totalzKcn);
    // Expanded
    ui->labelAvailablezKcn->setText(totalzKcn);
    ui->labelPendingzKcn->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));
    ui->labelImmaturezKcn->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
