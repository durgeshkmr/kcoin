...
public:
    ...
    bool WriteZKCNSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZKCNSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZKCNSeed_deprecated(uint256& seed);
    bool EraseZKCNSeed();
    bool EraseZKCNSeed_deprecated();

    bool WriteZKCNCount(const uint32_t& nCount);
    bool ReadZKCNCount(uint32_t& nCount);
