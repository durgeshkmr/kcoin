...
#include "zkcn/zkcnmodule.h"
#include "zkcn/zkcnwallet.h"
#include "zkcn/zkcntracker.h"
...
enum WalletFeature {
    ...
    FEATURE_PRE_KCOIN = 61000, // inherited version..
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4,           // ONLY_NONDENOMINATED and not 10000 KCN at the same time
    ONLY_10000 = 5,                                 // find masternode outputs including locked ones (use with caution)
    STAKEABLE_COINS = 6                             // UTXO's that are valid for staking
};

// Possible states for zKCN send
enum ZerocoinSpendStatus {
    ZKCN_SPEND_OKAY = 0,                            // No error
    ZKCN_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZKCN_WALLET_LOCKED = 2,                         // Wallet was locked
    ZKCN_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZKCN_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZKCN_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZKCN_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZKCN_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZKCN_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZKCN_TXMINT_GENERAL = 9,                        // General errors in MintsToInputVectorPublicSpend
    ZKCN_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZKCN_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZKCN_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZKCN_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZKCN_SPENT_USED_ZKCN = 14,                      // Coin has already been spend
    ZKCN_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZKCN_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    /// Get 10000 KCN output and keys which can be used for the Masternode
    ...
    bool CreateZKCNOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    // zKCN wallet
    CzKCNWallet* zwalletMain;
    std::unique_ptr<CzKCNTracker> zkcnTracker;
    void setZWallet(CzKCNWallet* zwallet);
    CzKCNWallet* getZWallet();
    ...
    // zKCN reset
    boost::signals2::signal<void()> NotifyzKCNReset;
};
