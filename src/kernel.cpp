...
bool initStakeInput(const CBlock& block, std::unique_ptr<CStakeInput>& stake, int nPreviousBlockHeight) {
    ...
    if (txin.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CLegacyZKcnStake(spend));
        ...
        if (nPreviousBlockHeight < consensus.height_start_ZC_SerialsV2 || nPreviousBlockHeight > consensus.height_last_ZC_AccumCheckpoint)
            return error("%s : zKCN stake block: height %d outside range", __func__, (nPreviousBlockHeight+1));
        CLegacyZKcnStake* zKCN = dynamic_cast<CLegacyZKcnStake*>(stake.get());
        if (!zKCN) return error("%s : dynamic_cast of stake ptr failed", __func__);
        // The checkpoint needs to be from 200 blocks ago
        const int cpHeight = nPreviousBlockHeight - consensus.ZC_MinStakeDepth;
        const libzerocoin::CoinDenomination denom = libzerocoin::AmountToZerocoinDenomination(zKCN->GetValue());
        if (ParseAccChecksum(chainActive[cpHeight]->nAccumulatorCheckpoint, denom) != zKCN->GetChecksum())
        ...
        CKcnStake* kcnInput = new CKcnStake();
        kcnInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(kcnInput);
    }
    return true;
}
