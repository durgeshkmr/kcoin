...
#if defined(NDEBUG)
#error "KCOIN cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ukcn) are considered zero fee (for relaying and mining)
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("%s : zKCN spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!txIn.IsZerocoinPublicSpend())
                    return state.Invalid(error("%s: failed for tx %s, every input must be a zcpublicspend",
                            __func__, tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zkcn");

                libzerocoin::ZerocoinParams* params = consensus.Zerocoin_Params(false);
                PublicCoinSpend publicSpend(params);
                if (!ZKCNModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)){
                    return false;
                }
                ...
                if (!CheckPublicCoinSpendVersion(publicSpend.getVersion())) {
                    return state.Invalid(error("%s : Public Zerocoin spend version %d not accepted. must be version %d.",
                            __func__, publicSpend.getVersion(), CurrentPublicCoinSpendVersion()), REJECT_INVALID, "bad-txns-invalid-zkcn");
                }
            ...
            // Reject legacy zKCN mints
            if (!Params().IsRegTestNet() && tx.HasZerocoinMintOutputs())
                return state.Invalid(error("%s : tried to include zKCN mint output in tx %s",
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZKCNStake)
{
    int64_t ret = 0;
    ...
    if (nHeight <= 43200) {
        ...
        //When zKCN is staked, masternode only gets 2 KCN
        ret = 3 * COIN;
        if (isZKCNStake)
            ret = 2 * COIN;
    }
        ...
        for (const CTxIn& in : tx.vin) {
            ...
            if (in.IsZerocoinSpend() || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZKCNModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from KCOIN
...
{
    util::ThreadRename("kcoin-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == consensus.height_ZC_RecalcAccumulators + 1) {
        RecalculateKCNSupply(consensus.height_start_ZC);
    }

    //Track zKCN money supply in the block index
    if (!UpdateZKCNSupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zKCN supply for block=%s height=%d", __func__,
...
bool CheckColdStakeFreeOutput(const CTransaction& tx, const int nHeight)
{
    ...
    if (outs >=3 && lastOut.scriptPubKey != tx.vout[outs-2].scriptPubKey) {
        ...
        // cold staking is active much after height_start_ZC_PublicSpends so GetMasternodePayment is always 3 KCN.
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zKCN spends in this block
        if (tx.HasZerocoinSpendInputs()) {
            for (const CTxIn& txIn : tx.vin) {
                bool isPublicSpend = txIn.IsZerocoinPublicSpend();
                if (txIn.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zKCN serial %s in block\n Block: %s",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Kcoin Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nKcoinBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nKcoinBadBlockBits)) {
            // accept KCOIN block minted with incorrect proof of work threshold
            return true;
        }
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> kcnInputs;
        std::vector<CTxIn> zKCNInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                zKCNInputs.push_back(stakeIn);
            }else{
                kcnInputs.push_back(stakeIn);
            }
        }
        const bool hasKCNInputs = !kcnInputs.empty();
        const bool hasZKCNInputs = !zKCNInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasKCNInputs) {
                    ...
                    for (const CTxIn& kcnIn : kcnInputs)
                        if(kcnIn.prevout == in.prevout)
        ...
        if (isBlockFromFork) {
            ...
            // Now that this loop if completed. Check if we have zKCN inputs.
            if(hasZKCNInputs) {
                for (const CTxIn& zKcnInput : zKCNInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zKcnInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex->nHeight, 0))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zkcn");
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // KCOIN: We use certain sporks during IBD, so check to see if they are
