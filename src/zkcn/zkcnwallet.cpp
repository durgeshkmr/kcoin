...
CzKCNWallet::CzKCNWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zkcn seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZKCNSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZKCNSeed_deprecated()) {
                    LogPrintf("%s: Updated zKCN seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zkcn seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fRegtest && fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zkcn wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzKCNWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZKCNCount(nCountLastUsed);
    else if (!walletdb.ReadZKCNCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzKCNWallet::Lock()
{
    seedMaster = 0;
}

void CzKCNWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzKCNWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZKCN(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzKCNWallet::LoadMintPoolFromDB()
{
...
void CzKCNWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzKCNWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzKCNWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zkcnTracker->HasPubcoinHash(pMint.first)) {
...
bool CzKCNWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZKCN(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zkcnTracker which also adds to database
    pwalletMain->zkcnTracker->Add(dMint, true);
...
void CzKCNWallet::SeedToZKCN(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
    libzerocoin::ZerocoinParams* params = Params().GetConsensus().Zerocoin_Params(false);
...
uint512 CzKCNWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzKCNWallet::GenerateDeterministicZKCN(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzKCNWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzKCNWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzKCNWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
