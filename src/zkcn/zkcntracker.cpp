// Copyright (c) 2018-2020 The KCOIN developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zkcn/deterministicmint.h>
#include "zkcntracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
#include "wallet/walletdb.h"
#include "zkcn/zkcnwallet.h"


CzKCNTracker::CzKCNTracker(std::string strWalletFile)
{
...
CzKCNTracker::~CzKCNTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzKCNTracker::Init()
{
...
bool CzKCNTracker::Archive(CMintMeta& meta)
{
...
bool CzKCNTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzKCNTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzKCNTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzKCNTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzKCNTracker::GetSerialHashes()
{
...
CAmount CzKCNTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzKCNTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzKCNTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzKCNTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzKCNTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzKCNTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzKCNTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzKCNTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzKCNTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzKCNTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzKCNTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzKCNWallet* zKCNWallet)
{
    bool iszKCNWalletInitialized = (NULL != zKCNWallet);
    CMintMeta meta;
    ...
    if (! iszKCNWalletInitialized)
        zKCNWallet = new CzKCNWallet(strWalletFile);
    meta.isSeedCorrect = zKCNWallet->CheckSeed(dMint);
    if (! iszKCNWalletInitialized)
        delete zKCNWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzKCNTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzKCNTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzKCNTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzKCNTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzKCNTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzKCNTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzKCNWallet* zKCNWallet = new CzKCNWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zKCNWallet);
        }
        delete zKCNWallet;
        LogPrint("zero", "%s: added %d dzkcn from DB\n", __func__, listDeterministicDB.size());
    }
