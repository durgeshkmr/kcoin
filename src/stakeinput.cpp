...
bool CKcnStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CKcnStake::GetTxFrom(CTransaction& tx) const
{
    tx = txFrom;
    return true;
}

bool CKcnStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CKcnStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CKcnStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream CKcnStake::GetUniqueness() const
{
    //The unique identifier for a KCN stake is the outpoint
...
CBlockIndex* CKcnStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;
