...
void PrepareShutdown()
{
    ...
    util::ThreadRename("kcoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "kcoin.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "kcoind.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the KCN and zKCN money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in KCN/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in KCN/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    std::string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, kcoin, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero, staking)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in KCN/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all KCOIN specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-kcnstake=<n>", strprintf(_("Enable or disable staking functionality for KCN inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zkcnstake=<n>", strprintf(_("Enable or disable staking functionality for zKCN inputs (0-1, default: %u)"), 1));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The KCOIN Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    util::ThreadRename("kcoin-loadblk");
...
/** Sanity checks
 *  Ensure that KCOIN is running in a usable environment with all
...
bool AppInit2()
{
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. KCOIN Core is shutting down."));
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. KCOIN Core is probably already running."), strDataDir));
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //KCOIN specific: zerocoin and spork DB's
                ...
                // KCOIN: load previous sessions sporks if we have them.
                ...
                if(Params().NetworkID() == CBaseChainParams::MAIN && chainHeight > consensus.height_last_ZC_WrappedSerials) {
                    ...
                    CAmount zkcnSupplyCheckpoint = consensus.ZC_WrappedSerialsSupply + GetWrapppedSerialInflationAmount();

                    if (pblockindex->GetZerocoinSupply() < zkcnSupplyCheckpoint) {
                        // Trigger reindex due wrapping serials
                        LogPrintf("Current GetZerocoinSupply: %d vs %d\n", pblockindex->GetZerocoinSupply()/COIN , zkcnSupplyCheckpoint/COIN);
                        reindexDueWrappedSerials = true;
                    } else if (pblockindex->GetZerocoinSupply() > zkcnSupplyCheckpoint) {
                        // Trigger global zKCN reindex
                ...
                if (GetBoolArg("-reindexmoneysupply", false) || reindexZerocoin) {
                    ...
                    RecalculateKCNSupply((reindexZerocoin ? consensus.height_start_ZC : 1), false);
                }
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of KCOIN Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart KCOIN Core to complete") << "\n";
        ...
        if (GetBoolArg("-upgradewallet", fFirstRun)) {
            ...
            if (prev_version <= FEATURE_PRE_KCOIN && pwalletMain->IsLocked()) {
        ...
        zwalletMain = new CzKCNWallet(pwalletMain->strWalletFile);
        ...
        if (zwalletMain->GetMasterSeed() != 0) {
            //Inititalize zKCNWallet
            uiInterface.InitMessage(_("Syncing zKCN wallet..."));

            //Load zerocoin mint hashes to memory
            pwalletMain->zkcnTracker->Init();
    ...
    /* Denominations
       ...
       For example:
       1KCN+1000 == (.1KCN+100)*10
       10KCN+10000 == (1KCN+1000)*10
    */
