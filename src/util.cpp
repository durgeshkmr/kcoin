...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "kcoin" is a composite category enabling all KCOIN-related debug output
            if (ptrCategory->count(std::string("kcoin"))) {
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty kcoin.conf if it does not exist
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "kcoind.pid"));
